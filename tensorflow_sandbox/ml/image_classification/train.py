...
def get_mnist_data(data_path, validation_ratio):
    ...
    train_idx, valid_idx = get_validation_split_indicies(len(train_x),
                                                         validation_ratio)
...
def train(data_path, validation_ratio, lr, epochs, dropout, l2_coef,
          model_path, debug):
    train_x, train_y, validation_x, validation_y, test_x, test_y = \
        get_mnist_data(data_path, validation_ratio)
    ...
    # model = FullyConnectedClassifier(input_size=(height * width),
    #                                  output_size=10,
    #                                  hidden_sizes=hidden_sizes)
    model = CNNClassifier(height=28, width=28, input_channels=1,
    ...
    with tf.Session() as sess:
        if not debug:
            ...
            validation_writer = tf.summary.FileWriter(
                logdir="./runs/{}_validation".format(start_time))

        sess.run(tf.group(tf.global_variables_initializer(),
                 tf.local_variables_initializer()))
        for epoch in range(epochs):
            if not debug:
                _, train_summary, train_loss, train_accuracy = \
                    sess.run([opt, merged, loss, accuracy],
                             feed_dict={x: train_x, y: train_y})
                train_writer.add_summary(train_summary, epoch)

                validation_summary, validation_loss, validation_accuracy = \
...
if __name__ == "__main__":
    ...
    train(args.data_path, args.validation_ratio, args.lr, args.epochs,
          args.dropout, args.l2_coef, args.model_path, args.debug)
